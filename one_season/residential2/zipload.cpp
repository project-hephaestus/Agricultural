...
ZIPload::ZIPload(MODULE *module) : residential_enduse(module)
{
	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_object,"virtual_battery",PADDR(virtual_battery),
			PT_int16,"only_once",PADDR(only_once), PT_DESCRIPTION, "Number of devices to model - base power is the total load of all devices",
			PT_double,"humidity",PADDR(humidity), PT_DESCRIPTION, "tha value of soil humidity, taken from soil sensor",
			PT_double,"humidity_setpoint",PADDR(humidity_setpoint), PT_DESCRIPTION, "the desired humidity setpoint of the user",
			PT_double,"actual_power_non_zero",PADDR(actual_power_non_zero),PT_DESCRIPTION,"this variable is used to from irrigation xontroller to make the bid",
			...
			PT_double,"prev_base_power[kW]",PADDR(prev_base_power), PT_DESCRIPTION, "base real power of the overall load",
			...
			PT_int16,"choose_load",PADDR(choose_load), PT_DESCRIPTION, "choose the load tha you want to use (0 or 1)",
