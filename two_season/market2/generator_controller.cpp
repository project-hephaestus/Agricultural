...
generator_controller::generator_controller(MODULE *module){
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_double, "bidprice", PADDR(bidprice), PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION, "the bid price",
			PT_double, "bid_quantity", PADDR(bid_quantity), PT_ACCESS, PA_REFERENCE, PT_DESCRIPTION, "the bid quantity",
			...
			PT_int16,"bid_from_now", PADDR(bid_from_now), PT_DESCRIPTION, "Current generator state as numeric value",
...
int generator_controller::init(OBJECT *parent)
{
	...
	else if (gl_object_isa(parent,"battery","generators"))	//Parent is a triplex node
	{
		//Grab the link to the constant_power properties
		ptemp = gl_get_property(parent,"Energy");
		give_now=gl_get_int16_by_name(parent,"give_now");
		//Check
		if ((ptemp == NULL) )
		{
			GL_THROW("generator_controller:%s failed to map the power property of the parent device",obj->name);
		...
		if (bid_curve_values[latency_write_section].valid_bid_period == true)
		{
			if (gen_mode == STANDALONE)
			{
				...
				for (index=0; index<bid_curve_values[latency_write_section].number_bid_curve_sections; index++)
				{
					...
					/////// NEW CONFIGURATION////////////////
					if((gl_object_isa(parent2,"diesel_dg","generators")))
					{
						bid_power_active=gl_get_double_by_name(parent2,"TotalRealPow");
						bid_power_reactive=gl_get_double_by_name(parent2,"TotalReacPow");
						bid_price=gl_get_double_by_name(parent2,"price");
						
					}
					else if(gl_object_isa(parent2,"windturb_dg","generators"))
					{
						no_wind=gl_get_double_by_name(parent2,"flag_no_wind");
					
						bid_power_active=gl_get_double_by_name(parent2,"TotalRealPow");
						bid_power_reactive=gl_get_double_by_name(parent2,"TotalReacPow");
						bid_price=gl_get_double_by_name(parent2,"price");
						bidprice=*bid_price;
						bid_quantity=*bid_power_active;
					...
					else if(gl_object_isa(parent2,"solar","generators"))
					{
						...
						bid_price=gl_get_double_by_name(parent2,"price");
						bidprice=*bid_price;
						bid_quantity=(*complex_power).Re();

									
					}
					else if(gl_object_isa(parent2,"battery","generators"))
					{
						double *emax=gl_get_double_by_name(parent2,"E_Max");

						bid_power_active=gl_get_double_by_name(parent2,"Energy");
						if(*bid_power_active <= *emax*0.2)
						{
						*bid_power_active=0;
						}
						bid_price=gl_get_double_by_name(parent2,"price");
						bidprice=*bid_price;
						bid_quantity=(*bid_power_active)/4;
					...
					if (gen_state != GEN_OFF)	//Generator is not off, so must be starting/started
					{
						...
						double bid = -1.0;
						//bid on KW
						if(gl_object_isa(parent2,"solar","generators"))
						{
							double contr_price=*bid_price;
							bidID_val=auction_object->submit(obj,((*complex_power).Re())/1000,contr_price,-1,BS_ON);
							//bidID_val2=auction_reac_object->submit(obj,((*complex_power).Im())/1000,bid_curve_values[latency_write_section].Curve_Info[index].price,-1,BS_ON);						
						}
						else if(gl_object_isa(parent2,"battery","generators"))
						{
							double *E_max=gl_get_double_by_name(parent2,"E_Max");
						//	printf("plaus2\\n\n\n");
							double contr_price=*bid_price;
							//if(*bid_power_active <  0){
							
						//	double *bid=gl_get_double_by_name(parent2,"E_Max");
							//*bid_power_active=*bid;
							//gl_set_value_by_name(parent2,"Energy","10000");
							//system("pause");
							//}
							if(first_time==2)
							{
								...
								{
								printf("edw");
								bidID_val=auction_object->submit(obj,(*bid_power_active)/1000,contr_price,-1,BS_ON);
								}
							}
					...
					{	
						...
						{
							double contr_price=*bid_price;
							bidID_val=auction_object->submit(obj,(((*complex_power).Re())/1000),contr_price,-1,BS_OFF);
						}
						else if(gl_object_isa(parent2,"battery","generators"))
						{						
							double contr_price=*bid_price;
						//	printf("plaus\\n\n\n\n\n");
							...
							if(first_time==2)
							{
								...
								{
									printf("edw");
								bidID_val=auction_object->submit(obj,((*bid_power_active)/1000),contr_price,-1,BS_OFF);
			...
			else	//Must be building mode
			{
				...
				else	//Only 1
				{
					...
					{	//Must be on or starting, so building is "off"
						...
						if(gl_object_isa(parent2,"solar","generators"))
						{
							double contr_price=*bid_price;
							bidID_val=auction_object->submit(obj,(-(*complex_power).Re())/1000,contr_price,-1,BS_OFF);
						//	bidID_val2=auction_reac_object->submit(obj,(-(*complex_power).Im())/1000,bid_curve_values[latency_write_section].Curve_Info[index].price,-1,BS_OFF);
