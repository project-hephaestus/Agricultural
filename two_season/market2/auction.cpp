...
auction::auction(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			PT_int16, "clearing_finished", PADDR(clearing_finished), PT_DESCRIPTION, "clearing_finished",
...
TIMESTAMP auction::presync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (t1>=clearat)
	{
		...
		clear_market();
		//printf("cleairgin_price%f\n",current_frame.clearing_price);
		// advance market_id
		++market_id;
...
void auction::clear_market(void)
{
	...
	//////////////////////////////////////
	/*
			double *cap_batter_virtual=gl_get_double_by_name(virtual_object,"capacity");
			double *price_virtual=gl_get_double_by_name(virtual_object,"price");
	
			if(current_frame.clearing_quantity >  *cap_batter_virtual)
			{
			gl_set_value_by_name(virtual_object,"capacity","0");
				
				 	gl_set_value_by_name(virtual_object,"charge","1");
			}
			else if(current_frame.clearing_price >*price_virtual)
			 {
				 printf("2	");
				  system("pause");
				 	gl_set_value_by_name(virtual_object,"capacity","0");
		    ...
		    double pos_x = *cap_batter_virtual-current_frame.clearing_quantity;
			sprintf(x_position_string, "%f", pos_x);
			gl_set_value_by_name(virtual_object,"capacity",x_position_string);
			gl_set_value_by_name(virtual_object,"charge","1");
			}
			system("pause");
			*/
	 ...
	 static FINDLIST *xt1=NULL;
				 ...
				 for(it1=firstt1;it1!=NULL;it1=it1->next)
				 {
					 ...
					 if(gl_object_isa(it1,"virtual_battery"))
				     {
						 ...
						 virtual_object=it1;
						gl_set_value_by_name(virtual_object,"change_capacity","1");
						gl_set_value_by_name(virtual_object,"ch","1");
					 }
