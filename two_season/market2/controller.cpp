...
controller::controller(MODULE *module){
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_int16, "second_period_for_market",PADDR(second_period_for_market),PT_DESCRIPTION, "flag whivh indicates when the residentrial participates into the market",
...
if(second_period_for_market==1)
{
	...
	//	printf("monitor:%f,min:%f max:%f, setpoint:%f\n",*pMonitor,min,max,setpoint0);
	//	// calculate bid price
		if(*pMonitor > setpoint0){
			//printf("right_side");
			k_T = ramp_high;
			T_lim = range_high;
		//	printf("values : %f %f %f ",bid,k_T, T_lim );
			...
			if(second_period_for_market==1)
			{
				...
				static FINDLIST *xt1=NULL;
				 ...
				 for(it1=firstt1;it1!=NULL;it1=it1->next)
				 {
				     ...
				     {
						 double *bid_price=gl_get_double_by_name(it1,"bid_price");
						 total=total+(*bid_price);
						 counter++;
				     }
			     }
		
			// system("pause");
				bid = *pAvg + ( (fabs(*pStd) < bid_offset) ? 0.0 : (*pMonitor - setpoint0) * (k_T * *pStd) / fabs(T_lim) ) - total/counter;
					if(bid<clear_price)
					{
						bid=clear_price;
					}
		...
		*/
		...
		// We might heat, if the price is right
			...
			if(*pMonitor != heating_setpoint0){
						...
						// system("pause");
							last_p = *pAvg + ( (fabs(*pStd) < bid_offset) ? 0.0 : (*pMonitor - heating_setpoint0) * ramp * (*pStd) / fabs(range) ) - total/counter;
		...
		// We might cool, if the price is right
			...
			if(*pMonitor != cooling_setpoint0){
				...
				if(second_period_for_market==1)
						...
						// system("pause");
							last_p = *pAvg + ( (fabs(*pStd) < bid_offset) ? 0 : (*pMonitor - cooling_setpoint0) * ramp * (*pStd) / fabs(range) )- total/counter;
								if(last_p<clear_price)
								{
									last_p=clear_price;
								}
...
}//end of second_period_for_market
	return TS_NEVER;
}
